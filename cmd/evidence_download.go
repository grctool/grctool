...
func runEvidenceDownload(cmd *cobra.Command, args []string) error {
	...
	for _, task := range tasks {
		...
		if !ok {
			ref = fmt.Sprintf("TASK-%s", task.ID)
		}

		// Tugboat attachment APIs key on the numeric Tugboat task ID. Domain
		// task IDs are strings (FEAT-004 ID unification); for Tugboat-sourced
		// tasks the string ID is the numeric Tugboat ID.
		taskIDInt, convErr := strconv.Atoi(task.ID)
		if convErr != nil {
			cmd.Printf("  ⚠️  Skipping task with non-numeric ID %q: %v\n", task.ID, convErr)
			stats.Errors++
			continue
		}
		...
		if window != "" {
			...
			attachments, err = tugboatClient.GetEvidenceAttachmentsByTaskAndWindow(ctx, taskIDInt, start, end)
			...
			attachments, err = tugboatClient.GetEvidenceAttachmentsByTaskAndWindow(ctx, taskIDInt, startDate, endDate)
		} else {
			// Get all attachments
			attachments, err = tugboatClient.GetEvidenceAttachmentsByTask(ctx, taskIDInt)
		}
...
func downloadAttachments(ctx context.Context, cmd *cobra.Command, cfg *config.Config, client *tugboat.Client, task *domain.EvidenceTask, ref string, attachments []tugboatModels.EvidenceAttachment, force bool, format string) *downloadStats {
	...
	for window, windowAttachments := range windowMap {
		// Create directory
		taskDirName := naming.GetEvidenceTaskDirName(task.Name, ref, task.ID)
