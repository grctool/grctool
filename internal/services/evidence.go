...
func (s *EvidenceServiceSimple) GenerateEvidence(ctx context.Context, req *EvidenceGenerationRequest) (*EvidenceGenerationResult, error) {
	...
	record := &domain.EvidenceRecord{
		ID:          fmt.Sprintf("ev_%s_%d", req.TaskID, time.Now().Unix()),
		TaskID:      req.TaskID,
		Title:       req.Title,
...
func (s *EvidenceServiceSimple) AnalyzeEvidenceTask(ctx context.Context, taskID string) (*EvidenceAnalysisResult, error) {
	// Get task details
...
func (s *EvidenceServiceSimple) saveGeneratedEvidence(evidence *models.GeneratedEvidence) error {
	...
	taskDir := filepath.Join(outputDir, fmt.Sprintf("ET%s", evidence.TaskID))
